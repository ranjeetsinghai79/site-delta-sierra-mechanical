...
export async function runAnalyticsAgent(
  ...
  try {
    ...
    const response = await client.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `Write a friendly weekly website performance summary for ${lead.name}.
Data: ${totalClicks} clicks, ${totalImpressions} impressions, top keywords: ${topKeywords}.
3-4 sentences. Encouraging tone. Mention what's working and one suggestion. No emojis.`,
      }],
    })
    ...
    const emailRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: process.env.OUTREACH_FROM_EMAIL || 'reports@builtai.dev',
        to: lead.email,
