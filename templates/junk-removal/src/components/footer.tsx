...
export default function Footer() {
  ...
  return (
    <footer className="bg-[var(--color-green-950)] text-white py-14 px-5">
      {/* Orange accent line */}
      <div className="w-full h-1 bg-[var(--color-orange-500)] mb-14" />

      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-10">
          {/* Brand */}
          <div>
            <div className="font-display font-900 text-xl uppercase tracking-wider mb-1 text-[var(--color-orange-400)]">
              Green Haul
            </div>
            <div className="font-body text-white/40 text-sm mb-4">Junk Removal · Tracy, CA</div>

            {/* Eco badge */}
            <div className="flex items-center gap-2 bg-[var(--color-green-900)]/60 border border-[var(--color-green-700)]/40 rounded-lg px-4 py-3 mb-4 max-w-xs">
              <Recycle className="w-5 h-5 text-[var(--color-green-400)] shrink-0" />
              <span className="font-body text-white/60 text-xs">
                85% of items donated or recycled — every haul.
              </span>
            </div>
          ...
          {/* Contact */}
          <div>
            <div className="font-display font-700 text-xs tracking-[0.25em] uppercase text-[var(--color-orange-400)] mb-4">
              Contact
            </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {BUSINESS.serviceAreas.map((area) => (
                <span key={area} className="text-white/45 text-xs font-body border border-white/10 px-3 py-1.5 rounded-lg">
                  {area}
                </span>
              ))}
            </div>
        ...
        <div className="border-t border-white/10 pt-7 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="font-body text-white/20 text-xs">
            Licensed · Insured · Eco-Friendly · {BUSINESS.license}
          </div>
        </div>
      </div>
    </footer>
